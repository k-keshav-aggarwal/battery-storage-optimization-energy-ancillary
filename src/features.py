...
def build_causal_features(datetimes, price_observed: np.ndarray,
    ...
    # Group by hour first, then shift(1) within group so hour t's reference
    # uses historical prices for the SAME hour of day from previous days.
    hod_ref = (p.groupby(dt.dt.hour)
                .transform(lambda x: x.shift(1).rolling(HOD_WINDOW_DAYS, min_periods=3).median()))
...
def expected_price_causal(datetimes, price_observed: np.ndarray) -> np.ndarray:
    ...
    hod = (p.groupby(dt.dt.hour)
            .transform(lambda x: x.shift(1).rolling(HOD_WINDOW_DAYS, min_periods=2).median()))
