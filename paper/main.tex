...
The 4-MAD reference flag is an objective benchmark in statistical scoring for detection, but actual trading gains are what matter. Furthermore, our dispatch optimization problem follows a linear degradation cost function of \$2.50 per MWh~\cite{Xu_BESS_Degradation, Peterson_BESS_Degradation} in the algorithm performance measures; incorporating the more realistic, non-linear rainflow count with electrochemical cell heat dynamics will improve long-term profits. In our reinforcement learning experiment, we observe that tabular Q-learning is robust, whereas DQN-based approximations in continuous spaces struggle with market disruptions. Lastly, we have experimented with the algorithm using only three price regions for spatial analysis, while a key future step is to experiment with graph neural networks on high-transmission topologies.
